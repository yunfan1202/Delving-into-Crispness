...
def main(running_file):
    ...
    args.datadir = "example_image"
    args.evaluate = "BSDS_refine_dice.pth"
    ...
    # multiscale_test(test_loader, model, args.start_epoch, running_file, args, blur=False)
...
def multiscale_test(test_loader, model, epoch, running_file, args, blur=False):

    from PIL import Image
    import scipy.io as sio
    model.eval()

    img_dir = os.path.join(args.savedir, 'imgs_epoch_%03d_ms' % (epoch - 1))
    # img_dir = args.savedir
    eval_info = '\nBegin to eval...\nImg generated in %s\n' % img_dir
    print(eval_info)
    running_file.write('\n%s\n%s\n' % (str(args), eval_info))
    ...
    for idx, (image, img_name) in enumerate(test_loader):
        ...
        if blur:
            image_in = cv2.GaussianBlur(image_in, (15, 15), 0)
